...
describe("Facade", () => {
	it("should return no elements when hotel has no rooms", () => {
		const facade = HotelFacade.fromOccupiedCells(new Set<string>(), HOTEL_SEED);

		expect(facade.elements).toEqual([]);
	});

	it("should return elements for cells adjacent to a single room", () => {
		const facade = HotelFacade.fromOccupiedCells(new Set(["0,1"]), HOTEL_SEED);
	...
	it("should produce deterministic results for the same grid and seed", () => {
		...
		const facadeA = HotelFacade.fromOccupiedCells(occupiedCells, HOTEL_SEED);
		const facadeB = HotelFacade.fromOccupiedCells(occupiedCells, HOTEL_SEED);

		expect(facadeA.elements).toEqual(facadeB.elements);
	});

	it("should return a parisTest wall for cell to the left of a room", () => {
		const facade = HotelFacade.fromOccupiedCells(new Set(["1,1"]), HOTEL_SEED);
	...
	it("should return ceiling and pipe for cell below a room", () => {
		const facade = HotelFacade.fromOccupiedCells(new Set(["0,2"]), HOTEL_SEED);
